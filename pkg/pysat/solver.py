...
class Solver:
    ...
    def run(self):
        ...
        return sat, spent, answer, self.print_list, self.table
    ...
    def solve(self):
        ...
        while not self.are_all_variables_assigned():
            ...
            if conf_cls is not None:
                ...
                self.print_list.append(('hr', 'black'))
                logger.info('--------decision level: %s ---------', self.level)
                self.print_list.append((f'DECISION LEVEL: {self.level}', 'b'))
                ...
                self.print_list.append(('picking {} to be {}'.format(bt_var, 'TRUE' if bt_val == TRUE else 'FALSE'), "darkmagenta"))
                logger.debug('branching variables: %s', self.branching_history)
                self.print_list.append(("branching variables:" + str(self.branching_history), "black"))
                self.table.append(self.assigns.copy())
                print(self.assigns)

            logger.debug('propagate variables: %s', self.propagate_history)
            self.print_list.append(('propagate variables: ' + str(self.propagate_history), "black"))
            logger.debug('learnts: \n%s', self.learnts)
            self.print_list.append(("learnts: " + str(self.learnts), "black"))
        return True
    ...
    def is_unit_clause(self, clause):
        ...
        values = []
        unassigned = None
        self.print_list.append(("-> Check if has an unit clause...", "chocolate"))
        ...
        if check:
            self.print_list.append(("FINDED: {}".format(unassigned), "darkgreen"))
            asg = str(self.assigns)
            asg = asg.replace("-1", "*")
            #self.print_list.append('assignments: {}'.format(asg))
        else:
            self.print_list.append(("NO FINDED", "darkred"))
    ...
    def unit_propagate(self):
        ...
        while True:
            ...
            self.print_list.append(('hr', 'black'))
            self.print_list.append(("BEGIN THE CHECK STEP", "b"))
            for clause in [x for x in self.cnf.union(self.learnts)]:
                c_val = self.compute_clause(clause)
                self.print_list.append(('clause: {}, value: {}'.format(self.convert_to_logic(list(clause)), c_val if c_val >= 0 else "*"), "darkblue"))
                if c_val == TRUE:
                    continue
                if c_val == FALSE:
                    return clause
            ...
            self.print_list.append(("propagate_queue: {}".format(propagate_queue), "crimson"))
            ...
            for prop_lit, clause in propagate_queue:
                ...
                self.print_list.append(("propagate {} to be {}".format(prop_var, self.assigns[prop_var]), "crimson"))
